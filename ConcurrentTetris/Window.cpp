...
void Window::drawGameboard(Gameboard* gameboard, Block* block_reference)
{
    for (int row = 0; row < gameboard->getRows(); row++) {
        for (int column = 0; column < gameboard->getColumns(); column++) {
            if (gameboard->isOccupied(row, column)) {
                sf::RectangleShape block(sf::Vector2f(block_reference->getHeight(), block_reference->getWidth()));
                block.setFillColor(gameboard->getGameboard()[row][column]->getColor());
                block.setPosition(column * block_reference->getWidth(), row * block_reference->getHeight() + SCOREBOARD_HEIGHT);
                window.draw(block);
            }
            else {
                sf::RectangleShape block(sf::Vector2f(block_reference->getHeight(), block_reference->getWidth()));
